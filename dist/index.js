...
(() => __awaiter(this, void 0, void 0, function* () {
    ...
    const crateTextureCanvas = yield readFile("src/assets/crate-texture.jpg");
    const stoneFloorTextureCanvas = yield readFile("src/assets/stone-floor-texture.jpg");
    ...
    const loop = (currentTime) => {
        ...
        const zbuf = new Array(canvas.width * canvas.height).fill(-100);
        for (const obj3d of objects3d) {
            for (let f of obj3d.faces) {
                ...
                if (imgt[0][2] > -0.1 || imgt[1][2] > -0.1 || imgt[2][2] > -0.1)
                    continue;
                const [xmin, xmax, ymin, ymax] = getBoundingBox(imgt);
                if (xmin > canvas.width || xmax < 0 || ymin > canvas.height || ymax < 0)
                    continue;
